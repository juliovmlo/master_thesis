...
def rot_mat_2_vec(v1,v2):
    ...
    #
    c = np.dot(v1,v2)
    v = np.matmul(v_tilde(v1), v2)
    aa =np.linalg.norm(v)
    #
    R = np.eye(3) + v_tilde(v) + np.matmul(v_tilde(v),v_tilde(v)) * 1/(1+c)
    return R
#
...
def quaternion_from_rotation_tensor(R):
    # See page 65
    r = np.zeros((4))
    S = np.diag(np.array([1+R[0,0]+R[1,1]+R[2,2],
                       1+R[0,0]-R[1,1]-R[2,2],
                       1-R[0,0]+R[1,1]-R[2,2],
                       1-R[0,0]-R[1,1]+R[2,2]]))
    S[1,0] = S[0,1] = R[2,1] - R[1,2]
    S[2,0] = S[0,2] = R[0,2] - R[2,0]
    S[3,0] = S[0,3] = R[1,0] - R[0,1]
    S[2,1] = S[1,2] = R[1,0] + R[0,1]
    S[3,1] = S[1,3] = R[0,2] + R[2,0]
    S[3,2] = S[2,3] = R[1,2] + R[2,1]

    # i = np.argmax(np.diag(S))
    # r[i] = 0.5 * np.sqrt(S[i,i])
    for j in range(4):
        r[j] = 0.5 * np.sqrt(S[j,j])
        # r[j] = 0.25 * S[i,j] / r[maxComp]
    return r
...
def add_two_quaternion(p,q):
    ...
    r[0] = p0*q0 - np.dot(p_vec.T,q_vec)
    r[1:] = p0*q_vec + q0*p_vec + np.cross(p_vec,q_vec)
    return r
