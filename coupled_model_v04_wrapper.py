...
def coupled_model (U0,omega,pitch_deg,htc_filename, projectfolder, resfolder=None, epsilon=1e-3, iter_max=20):
    ...
    while (epsilon is None or abs(delta_u_rel) > epsilon) and iter < iter_max:
        # Update variables
        pos_old = pos_new
        ...
        # We shoud do
        # delta_u_rel = np.linalg.norm(new_tip_def - old_tip_def)/ np.linalg.norm(old_tip_def)
        ...
        print(f"Delta = {delta_u_rel*100:.5f}%")
    ...
    sim_name = os.path.splitext(os.path.basename(htc_filename))[0]
    if resfolder is None:
        resfolder = os.path.join(projectfolder,f'res/res_{sim_name}.json')
    else:
        resfolder =  os.path.join(resfolder,'results.json')
    save_results(results_dict,resfolder)
